...
function resolveBinary() {
  ...
  if (!fs.existsSync(fullPath)) {
    const hint = isTermux()
      ? '  On Termux: pkg install nodejs && npm install -g @skyripper/deepcrypt\n' +
        '  Or (git clone): bash install.sh'
      : platform === 'win32'
        ? '  Run build/build.ps1 to compile from source.'
        : '  Run bash install.sh  (git-clone install, no npm needed)\n' +
          '  Or build from source: bash build/build.sh';
    fatal(
...
const GITHUB_RAW_PKG =
  'https://raw.githubusercontent.com/skyripper-arch/Deepcrypt/main/package.json';
...
async function handleUpdate() {
  ...
  try {
    ...
    console.error('[dpc] npm update failed. Alternatives:');
    console.error('  git clone install : git pull && bash install.sh');
    console.error('  npm               : npm install -g @skyripper/deepcrypt');
    process.exit(1);
  }
}
